...
export default function Footer() {
  ...
  return (
    <footer className="">
      <div className="bg-neutral-100 flex flex-col md:flex-row gap-4 justify-between px-4 md:px-16 py-16">
        <div className="flex flex-col items-center md:items-start">
          <Logo />
          <div className="mt-12 flex flex-col gap-4">
            <p className="flex gap-1">
              <IconLocation /> Address ABC, New York,
              <br /> Unites States
            </p>
            <p className="flex gap-1">
              <IconPhone /> Help Line & Whatsapp: <br /> +123 456 789
            </p>
            <p className="flex gap-1">
        ...
        <div className="flex flex-col gap-3 font-medium text-center md:text-left mt-8 md:mt-0">
          ...
          {quickLinks.map((item) => (
            <Link className="hoverable" href={item.link}>
              {item.name}
            </Link>
          ))}
        </div>
      ...
      <div className="bg-neutral-300 p-4">
        <p className="text-center">
          © Copyright 2023 Vision Vault All rights reserved.
        </p>
      </div>
    </footer>
  );
